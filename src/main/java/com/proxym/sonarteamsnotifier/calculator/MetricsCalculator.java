...
public class MetricsCalculator {
    ...
    protected static final List<MetricDetails> metrics;
    ...
    public static CalculatorResponse calculate(String baseUrl, String noPageDefinitionUrl, String token) {
        ...
        return new CalculatorResponse(firstScan, measures.stream().filter(measure -> !measure.getHistory().isEmpty()).map(measure -> {
                    ...
                    if (isNumber(recentValue)) {
                        ...
                        measureDifference = previousValue.orElse("");
                    }
                    }
    ...
    private static List<Measure> provideMeasures(String baseUrl, String noPageDefinitionUrl, String token) {
        ...
        if (total == 1) {
        measures.addAll(
            SonarRequestSender.get(baseUrl, noPageDefinitionUrl + String.format(PayloadUtils.PAGINATION_PARAMS, 1, 1), token).getMeasures());
        }
        else {
            measures.addAll(SonarRequestSender.get(baseUrl, noPageDefinitionUrl + String.format(PayloadUtils.PAGINATION_PARAMS, 2, total / 2), token).getMeasures());
            if (total % 2 != 0) {
                List<Measure> lastScan = SonarRequestSender.get(baseUrl, noPageDefinitionUrl + String.format(PayloadUtils.PAGINATION_PARAMS, 2, total / 2 + 1), token).getMeasures();
                measures.forEach(measure -> measure.getHistory().addAll(lastScan.stream().filter(lastMeasure -> lastMeasure.getMetric().equals(measure.getMetric())).findFirst().orElseThrow(MetricNotFoundException::new)
                        .getHistory()));
            }
        }
    ...
    private static void assignColor(MeasureDto measureDto, double difference, boolean higherValuesAreBetter) {
        if (measureDto.getMetric().equals(PayloadUtils.ALERT_STATUS_METRIC)) {
            measureDto.setOrder(100);
            if (measureDto.getDifference().equals("ERROR")) {
                measureDto.setColor(Color.RED.getCssStyle());
            } else {
                measureDto.setColor(Color.GREEN.getCssStyle());
            }
        } else if (measureDto.getMetric().equals("lines")){
